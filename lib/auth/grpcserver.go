...
func (g *GRPCServer) TryAcquireSemaphore(ctx context.Context, req *proto.AcquireSemaphore) (*services.SemaphoreLease, error) {
	auth, err := g.authenticate(ctx)
	if err != nil {
		return nil, trail.ToGRPC(err)
	}
	lease, err := auth.TryAcquireSemaphore(ctx, req.Semaphore, *req.Lease)
...
func (g *GRPCServer) GetAllSemaphores(ctx context.Context, _ *empty.Empty) (*proto.Semaphores, error) {
	auth, err := g.authenticate(ctx)
	if err != nil {
		return nil, trail.ToGRPC(err)
	}
	semaphores, err := auth.GetAllSemaphores(ctx)
	if err != nil {
		return nil, trail.ToGRPC(err)
	}
...
/*func (g *GRPCServer) DeleteAllSemaphores(ctx context.Context, _ *empty.Empty) (*empty.Empty, error) {
	auth, err := g.authenticate(ctx)
	if err != nil {
		return nil, trail.ToGRPC(err)
	}
	if err := auth.DeleteAllSemaphores(ctx); err != nil {
		return nil, trail.ToGRPC(err)
	}
	return &empty.Empty{}, nil
}*/
