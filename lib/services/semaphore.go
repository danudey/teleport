...
// SemaphoreKindUserSession is the semaphore kind used to track
// and limit concurrent user session session.
const SemaphoreKindUserSession = "user_session"
...
type Semaphores interface {
	...
	TryAcquireSemaphore(ctx context.Context, params AcquireSemaphore) (*SemaphoreLease, error)
...
// AcquireSemaphoreLease attempts to acquire a semaphore lease.  If successfully acquired, background
// keepalive processes are started and an associated lock handle is returned.
func AcquireSemaphoreLease(ctx context.Context, cfg SemaphoreLockConfig) (*SemaphoreLock, error) {
	...
	lease, err := cfg.Service.TryAcquireSemaphore(ctx, cfg.Params)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	...
	go func(lease SemaphoreLease) {
		defer func() {
			...
			if lease.Expires.After(lock.cfg.Clock.Now()) {
		...
		ticker := lock.cfg.Clock.NewTicker(lock.cfg.Expiry / 2)
		defer ticker.Stop()
	Outer:
		for {
			select {
...
func (s *AcquireSemaphore) CheckAndSetDefaults() error {
	if s.SemaphoreKind == "" {
...
func (s *AcquireSemaphore) lease() *SemaphoreLease {
...
func (s *AcquireSemaphore) leaseRef() SemaphoreLeaseRef {
...
func (s *AcquireSemaphore) ConfigureSemaphore() (Semaphore, error) {
	sem := SemaphoreV3{
		Kind:    KindSemaphore,
...
type Semaphore interface {
	...
	Acquire(params AcquireSemaphore) (*SemaphoreLease, error)
...
func (c *SemaphoreV3) Acquire(params AcquireSemaphore) (*SemaphoreLease, error) {
	...
	if c.LeaseCount() >= params.MaxLeases {
		return nil, trace.LimitExceeded("cannot acquire semaphore (leases=%d, limit=%d)", c.LeaseCount(), params.MaxLeases)
	}
